...
function normalizarLogros(userAch = [], schemaAch = []) {
  const mapSchema = new Map(schemaAch.map((s) => [s.name, s]));

  return userAch.map((logro) => {
    const meta = mapSchema.get(logro.apiname);
    return {
      id: logro.apiname,
      name: logro.name || meta?.displayName || logro.apiname,
      image: meta?.icon || "",
      lockedImage: meta?.icongray || "",
      description: logro.description || meta?.description || "",
...
function detectarNuevosLogros(prev = [], current = []) {
  ...
  const prevMap = new Map(prev.map((l) => [l.id, l.achieved === 1]));

  return current.filter((l) => l.achieved === 1 && prevMap.get(l.id) === false);
}
...
app.get("/api/steam/achievements", async (req, res) => {
  try {
    ...
    const playtime = await obtenerTiempoDeJuego(
      juego.appid,
      STEAM_KEY,
      STEAM_ID,
    );
    const logros = await obtenLogros(
      juego.appid,
      STEAM_KEY,
      STEAM_ID,
      LANGUAGE,
    ...
    const payload = {
      ...
      lastAchievements: ultimos || {
        ...
        image:
          "https://widget.chemita.dev/steam-tracker/resource/steam_logo.jpg",
      },
